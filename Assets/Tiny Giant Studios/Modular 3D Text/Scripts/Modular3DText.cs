...
namespace MText
{
    public class Modular3DText : MonoBehaviour
    {
        ...
        public bool autoCreateInPlayMode = false;
        ...
        //Layout Settings----------------------------------------------------------------------------------------
        public int layoutStyle = 0; // false = circular
        int oldLayoutStyle = 0; // false = circular
            ...
            for (int i = 0; i < wordList.Count; i++)
            {
                ...
                else if (currentWordSpacing > adjustedLength)
                {
                    ...
                    else
                    {
                        ...
                        {
                            float mySpacing = font.Spacing(c) * characterSpacing;
        ...
        float TotalSpacingRequiredFor(string myString)
        {
            ...
            foreach (char c in chars)
            {
                totalSpace += characterSpacing * font.Spacing(c);
            }
                ...
                if (myCounter + lineList[i].Length > startingFrom)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                x += characterSpacing * font.Spacing(c) * textDirection;
                            }
                        }
        ...
        GameObject GetObject(char c)
        {
            ...
            if (activateChildObjects) obj.SetActive(true);
            else obj.SetActive(false);
            return obj;
        }
        //Doesn't run In Editor mode
        ...
        void AddCharacterToList(GameObject obj)
        {
        ...
        bool GetPaths()
        {
            ...
            {
                string path = EditorUtility.SaveFilePanel("Save Separate Mesh" + i + " Asset", "Assets/", name + " mesh " + i, "asset");
                if (string.IsNullOrEmpty(path))
                {
                    return true;
                }
                else path = FileUtil.GetProjectRelativePath(path);
                meshPaths.Add(path);
            }
            return false;
        }
